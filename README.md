...
Demonstrates dynamic locale switching in CE.SDK. Switch between English and German to see the entire editor UI update in real-time. Built with [CE.SDK](https://img.ly/creative-sdk) by [IMG.LY](https://img.ly), runs entirely in the browser with no server dependencies.

<p>
  <a href="https://img.ly/docs/cesdk/web/ui-styling/localization/">Documentation</a> |
...
```bash
git clone https://github.com/imgly/starterkit-translation-internationalization-ts-web.git
cd starterkit-translation-internationalization-ts-web
```
...
```
starterkit-translation-internationalization-ts-web/
├── src/
│   ├── index.ts              # Application entry point with locale switcher
│   └── imgly/
│       ├── index.ts          # Editor initialization with locale setup
│       ├── config/
